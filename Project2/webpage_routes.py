...
def display_billing():
    #response = requests.get('http://localhost:5005/billing-query')
    ...
    #if not rows:
        # If rows is empty, return an HTML page or segment with "No bookings"
        #return render_template('billing.html', table_html='<p>No Records</p>')
    ...
    #try:
        # Attempt to send the filtered rows to another service to format them as HTML table
        #format_response = requests.post('http://localhost:5006/billing_table', json={'rows': rows}, timeout=5)
        #if format_response.status_code == 200:
            #table_html = format_response.text
        #else:
            # Log error and use default HTML
...
def register():
    # Attempt to fetch data from the homepage-query service
    response = requests.get('http://localhost:5005/provider-query')
    if response.status_code == 200:
